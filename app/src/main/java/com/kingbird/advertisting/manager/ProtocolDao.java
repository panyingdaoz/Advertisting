package com.kingbird.advertisting.manager;
...
import static com.kingbird.advertisting.base.Base.intToButeArray;
import static com.kingbird.advertisting.base.Base.strToByteArray1;
import static com.kingbird.advertisting.base.Base.toBytes;
import static com.kingbird.advertisting.utils.Config.APK_CHECK;
import static com.kingbird.advertisting.utils.Config.CONSTANT_EIGHT;
...
import static com.kingbird.advertisting.utils.Config.CONSTANT_THIRTEEN;
...
public class ProtocolDao {
     ...
     * 帧头
     */
    private static final short HEADER_NORMAL = (short) 0xA881;
...
//        String cpuID = getCpuSerial();
        String cpuId = MacUtil.getDeviceId();
        int cpuIdLength = cpuId.length();
        Plog.e("cupID和长度", cpuId + "++" + cpuIdLength);

        byte[] data = new byte[57 + cpuIdLength];
        ...
        //数据长度
        byte[] bytes = hexString2Bytes(intToHexString(37 + cpuIdLength, 2));
        ...
        data[54] = (byte) 1;
        data[55] = (byte) deviceType;

//        System.arraycopy(toBytes(cpuId), 0, data, 56, cpuIdLength);38363732323330323537303339313
        System.arraycopy(cpuId.getBytes(), 0, data, 56, cpuIdLength);

        //校验
        data[56 + cpuIdLength] = calcCheckSum(data, data.length - 1);
