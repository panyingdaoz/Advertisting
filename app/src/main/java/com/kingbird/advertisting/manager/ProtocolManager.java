...
//    }
     ...
     * 通讯类型应答
     */
    private void netDataAnser(byte[] data) {
        String networkingProtocol = "tcp";
        String protocolTpye = SpUtil.readString(Const.NET_TYPE);
        e("协议类型", protocolTpye);
        if (networkingProtocol.equals(protocolTpye)) {
            int sendLength = SocketManager.getInstance().send(data);
            if (sendLength == 0 && NetUtil.isNetConnected(context)) {
    ...
    public void sendHeartBeat() {
        ...
        longitudeStr = readString(Const.LONGITUDE);
        latitudeStr = readString(Const.LATITUDE);
        ...
        try {
            ...
            for (Parameter parameters : parameter) {
                ...
                if (id != null) {
                    ...
                    SpUtil.writeString(Const.DEVICE_ID, id);
                    SpUtil.writeString(Const.RCODE, rCode);
                    e("最终Mac地址", mac);
